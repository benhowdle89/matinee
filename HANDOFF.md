...
- **Two doc assets need a browser to regenerate.** Everything else is pure Node.

  `assets/og.png`: `npm run og` writes `assets/og.svg`; turning it into a PNG
  was done by screenshotting it in Chrome via a throwaway Playwright script.
  To redo it: `npx playwright` + `page.goto(file://…/og.svg)` + `screenshot`,
  viewport 1200x630, `deviceScaleFactor: 1`.

  `assets/export-path.png`: `toPathPng()` needs a real 2D canvas, which Node
  does not have. It was generated by serving the repo over `python3 -m
  http.server`, bundling `src/export-png.ts` standalone with esbuild (that file
  imports no React, so it loads in a browser as a plain module), and calling it
  from a page. The bundled `dist/index.js` will *not* load directly in a
  browser because it imports `react` as a bare specifier.

  matinee gains no dependency for either, which is the point. Both are
  regenerated approximately never.
...
Three plain-text files, served from the demo site and mirrored at the repo
root, generated by `npm run llms`:

| File | What it is |
|---|---|
| `llms.txt` | The index, per llmstxt.org. Short, points at everything else. |
| `llms-full.txt` | The whole documentation set in one file: usage, recipes, surprising behaviours, an explicit list of what does not exist, and the shipped `dist/index.d.ts` verbatim. |
| `agent-prompt.txt` | The "paste this into your coding agent" brief. |

Two decisions worth keeping:

**The API section is the real `.d.ts`, not a summary.** Declarations carry their
JSDoc, models read TypeScript fluently, and it cannot drift from what people
install. CI regenerates the files and fails if the committed copies differ, so
an API change cannot quietly leave the docs behind.

**The "what matinee is not" list earns its place.** Without it a code generator
will confidently invent `cursor.drag()`, a GIF export or a Vue adapter, because
those are all things a library like this plausibly would have. Naming the
absences is more useful to a model than any amount of describing what is there.

The agent prompt lives in `demo/agent-prompt.ts` and is the single source of
truth. The generator extracts it, writes the text file, and asserts the README
contains the identical string, so the three copies cannot diverge without
failing the build. If you edit the prompt, edit it there and run `npm run llms`.
...
npm run docs:assets   # rebuild the personality and export-example SVGs
npm run og            # rebuild assets/og.svg
npm run llms          # rebuild llms.txt, llms-full.txt and agent-prompt.txt
```
